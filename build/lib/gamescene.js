    ...
    constructor() {
      ...
      if (SERVER) {
        ...
        network.addEventListener('usermove', event => {
          if (this.snapshotMap[event.tick] && event.spark.entId && this.snapshotMap[event.tick].entities[event.spark.entId] && this.snapshotMap[event.tick].entities[event.spark.entId].vars) {
